...
export default function ConnectAssetModal({ onClose, onConnect, connections = [], onRemoveConnection }) {
  ...
  React.useEffect(() => {
    setProvider(""); setApiKey(""); setApiSecret(""); setError("");
  }, [category]);
  ...
  function handleConnect() {
    if (!provider)        { setError("거래소를 선택하세요"); return; }
    if (!apiKey.trim())   { setError("Access key를 입력하세요"); return; }
    if (!apiSecret.trim()) { setError("Secret key를 입력하세요"); return; }
    setLoading(true); setError("");
    setTimeout(() => {
      setLoading(false);
      onConnect({ category, provider, apiKey, apiSecret, connectedAt: new Date().toISOString() });
      setProvider(""); setApiKey(""); setApiSecret("");
    }, 600);
  }
    ...
    <>
        ...
        <div className="gu-modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="gu-modal-body">
            ...
            {provider && (
              <>
                <div className="gu-modal-field">
                  ...
                  <input type="text" className="gu-input" value={apiKey}
                         onChange={e => setApiKey(e.target.value)}
                         placeholder="Access key를 입력하세요" disabled={loading}/>
                </div>
                <div className="gu-modal-field">
                  ...
                  <div className="gu-input-wrapper">
                    <input type={showSecret ? "text" : "password"} className="gu-input"
                           ...
                           placeholder="Secret key를 입력하세요" disabled={loading}/>
                ...
                <div className="gu-connections-table-wrap">
                  <table className="gu-table gu-connections-table">
                    ...
                    <tbody>
                      {connections.map((c, i) => (
                        <tr key={i}>
                          <td><div className="gu-connection-name">{providerName(c.category, c.provider)}</div></td>
            ...
            <div className="gu-modal-note">
              <strong>보안 주의:</strong> API 키와 시크릿은 암호화되어 로컬에만 저장됩니다. 외부에 공유하지 마세요.
            </div>
          </div>

          <div className="gu-modal-actions">
            <button className="gu-btn gu-btn-secondary" onClick={onClose} disabled={loading}>닫기</button>
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          onConfirm={() => { onRemoveConnection(confirmDelete.idx); setConfirmDelete(null); }}
