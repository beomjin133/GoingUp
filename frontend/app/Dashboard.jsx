// Dashboard — hero + allocation + holdings table

function Dashboard({ state, dispatch, data, onRefresh }) {
  ...
  const equity = React.useMemo(() => {
    if (snapshots.length >= 1) {
      return [...snapshots.map(d => d.value), total];
    }
  ...
  // Format current time as "YYYY-MM-DD HH:MM:SS KST"
    ...
    <main className="gu-page gu-fade-in">
      ...
      <div className="gu-kpi-grid">
        {[
          ...
          { label: "예수금",   v: byKind.cash,   pct: byKind.cash / total * 100,   color: "#6B7684" },
        ].map((k, i) => (
      ...
      {/* Asset donut breakdown */}
      <AssetDonutSection holdings={holdings} total={total} hidden={hidden} />
...
function AssetDonutSection({ holdings, total, hidden }) {
  const KINDS = [
        ...
        if (items.length === 0) return (
          <div key={k.id} className="gu-donut-card gu-donut-card-empty">
            <div className="gu-donut-card-title">
              <span className="gu-donut-card-dot" style={{background: k.color}}/>
              {k.label}
            </div>
            <span>보유 종목 없음</span>
          </div>
          ...
          <div key={k.id} className="gu-donut-card">
            ...
            <div className="gu-donut-card-body">
              <Donut
                ...
                centerLabel={k.label}
                centerValue={hidden ? "••••" : fmtShortKRW(kindTotal)}
              />
          ...
          <tbody>
            {rows.map(h => {
              ...
              return (
                ...
                <tr
                    className={(isUp ? "is-up" : "is-down") + (big ? " is-big" : "") + (isOpen ? " is-open" : "")}
                    onClick={(e) => {
                      // If clicking the chevron, toggle; otherwise go detail
                  ...
                  <td>{h.amt.toLocaleString("ko-KR", { maximumFractionDigits: 4 })}</td>
...
function LotsPanel({ h, hidden, onSellLot }) {
          ...
          return (
            <div key={l.id} className="gu-lots-grid-row">
              <div className="gu-lot-id" title={l.id}>
                <span className="gu-lot-id-dot" style={{background: h.color}}/>
                <code>{l.id.slice(0, 8)}…</code>
              </div>
              <div className="gu-num" style={{color:"var(--gu-fg2)", fontSize:11}}>{l.date.slice(5)}<span style={{color:"var(--gu-fg4)", marginLeft:6}}>{l.time}</span></div>
              <div className="gu-num" style={{textAlign:"right"}}>{amt.toLocaleString("ko-KR", { maximumFractionDigits: 4 })}</div>
